        ...
        public async Task<BaseResponse<DynamicsTrackingNoLogin>> TrackingByIDTRA(string idtra)
        {
            ...
            try
            {
                ...
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    HttpResponseMessage httpResponseMessaje = await httpClient.GetAsync($"api/data/v9.2/{entityName}?$select=new_etadestino,new_contenedor,new_bcf,new_confirmacinzarpe,new_destino,new_eta,new_etd1,new_instcliente,new_ingreso,new_ingresoabodegas,new_barcodesalida,new_origen,new_poe,new_pol,new_transporte,title,new_preestado2,modifiedon&$filter=contains(title,'{idtra}')&$orderby=title asc");
